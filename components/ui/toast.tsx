...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      'fixed bottom-4 right-4 top-auto z-[100] flex max-h-screen w-[calc(100vw-2rem)] max-w-[360px] flex-col gap-2 p-0 sm:flex-col',
      className
    )}
    {...props}
  />
...
const toastVariants = cva(
  'group pointer-events-auto relative flex w-full items-start justify-between gap-3 overflow-hidden rounded-lg border p-4 pr-9 shadow-lg backdrop-blur transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-bottom-3',
  {
    variants: {
      variant: {
        default: 'border-[var(--border2)] bg-[var(--card)] text-[var(--text)]',
        destructive:
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description
    ref={ref}
    className={cn('text-xs leading-snug text-[var(--muted)]', className)}
    {...props}
  />
))
